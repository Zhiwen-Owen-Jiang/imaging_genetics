...
class VariantSetTest:
    def __init__(self, bases, resid_ldr, covar, chr=None, loco_preds=None):
        """
        ...
        if loco_preds is not None and chr is not None:
        ...
        self.var = np.sum(np.dot(bases, inner_ldr) * bases, axis=1) / (
            self.n - self.p
        )  # (N, )
...
def prepare_vset_test(snps_mt_cate):
    ...
    vset = BlockMatrix.from_entry_expr(
        snps_mt_cate.flipped_n_alt_alleles, mean_impute=True
    )  # (m, n) slow
