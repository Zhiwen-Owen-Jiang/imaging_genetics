...
class VariantSetTest:
    def __init__(self, bases, inner_ldr, ldrs, covar):
        ...
        self.var = np.sum(np.dot(self.bases, self.inner_ldr) * self.bases.T, axis=1) / (self.n - self.p) # (N, )
    ...
    def input_vset(self, vset, annotation_pred, rare_maf_cutoff=0.01):
        ...
        if len(self.maf) < 2:
            raise ValueError(f'Too few rare variants after screened by cutoff {rare_maf_cutoff}.')
        self.vset = vset[:, self.maf < rare_maf_cutoff] # TODO: read as a column sparse matrix
    ...
    def _get_weights(self, annot=None):
        ...
        if annot is None:
            ...
            weights_dict['skat'] = self._combine_weights(w1, w2, np.sqrt(annot_rank))
    ...
    def _skat_test(self, weights):
        ...
        score_stat = np.sum(np.dot(self.bases, ldr_score) * self.bases.T, axis=1) / self.var # (N, )

        wcov_mat = weights.reshape(-1, 1) * self.cov_mat * weights # (m, m)
        egvalues, _ = np.linalg.eigh(wcov_mat)
        egvalues = np.flip(egvalues).reshape(1, -1) # (m, ) all voxels share the same eigenvalues
    ...
    def _burden_test(self, weights):
        ...
        """
        burden_score_num = np.dot(np.dot(weights, self.half_ldr_score), self.bases.T) ** 2 # (N, ) 
        burden_score = burden_score_num / (self.var * np.dot(np.dot(weights, self.cov_mat), weights.T))# (N, )
    ...
    def _acatv_test(self, weights):
        ...
        common_variant_pv = chi2.sf((half_score[~self.is_rare] ** 2 / denom[~self.is_rare]), 1) # (m1, N)
        
        rare_burden_score_num = np.dot(weights[self.is_rare], half_score[self.is_rare]) ** 2 # (N, )
        rare_burden_score_denom = self.var * np.dot(np.dot(weights[self.is_rare], self.cov_mat[self.is_rare][:, self.is_rare]), weights[self.is_rare].T) # (N, )
    ...
    def do_inference(self):
        ...
        results_STAAR_O = cauchy_combination(np.hstack([skat_pvalues, burden_pvalues, acatv_pvalues]), axis=1)
...
def cauchy_combination(pvalues, weights=None, axis=0):
    ...
    stats = np.mean(np.tan((0.5 - pvalues) * np.pi) * normed_weights, axis=axis)
