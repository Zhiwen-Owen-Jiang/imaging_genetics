...
class Test_partition_genome(unittest.TestCase):
    def test_partition_genome_case1(self):
        ...
        """
        ldinfo = pd.DataFrame({'CHR': [1, 1, 1, 1, 1],
                               'POS': [100, 200, 300, 400, 500]})
        part = pd.DataFrame({0: [1, 1, 1, 1, 1],
                             1: [100, 200, 300, 400, 500],
                             2: [200, 300, 400, 500, 600]})
        true_num_snps_part = [2, 1, 1, 1]
        true_ld_bim = pd.DataFrame({'CHR': [1, 1, 1, 1, 1],
                                    'POS': [100, 200, 300, 400, 500],
                                    'block_idx': [0, 0, 1, 2, 3],
    ...
    def test_partition_genome_case2(self):
        ...
        """
        ldinfo = pd.DataFrame({'CHR': [1, 1, 1, 1],
                               'POS': [100, 200, 300, 400]})
        part = pd.DataFrame({0: [1, 1, 1, 1],
                             1: [100, 200, 300, 500],
                             2: [200, 300, 500, 600]})
        true_num_snps_part = [2, 1, 1]
        true_ld_bim = pd.DataFrame({'CHR': [1, 1, 1, 1],
                                    'POS': [100, 200, 300, 400],
                                    'block_idx': [0, 0, 1, 2],
                                    'block_idx2': [0, 1, 0, 0]})
        num_snps_part, ld_bim = partition_genome(ldinfo, part, log)
    ...
    def test_partition_genome_case3(self):
        ...
        ldinfo = pd.DataFrame({'CHR': [1, 1, 1, 1],
                               'POS': [100, 200, 300, 400]})
        part1 = pd.DataFrame({0: [1, 1, 1, 1],
                             1: [1000, 2000, 3000, 5000],
                             2: [2000, 3000, 5000, 6000]})
        part2 = pd.DataFrame({0: [1, 1, 1, 1],
                             1: [10, 20, 30, 50],
                             2: [20, 30, 50, 60]})
        with self.assertRaises(ValueError):
    ...
    def test_partition_genome_case4(self):
        ...
        """
        ldinfo = pd.DataFrame({'CHR': [1] * 3000,
                               'POS': list(range(0, 6000, 2))})
        part = pd.DataFrame({0: [1, 1],
                             1: [0, 5000],
                             2: [5000, 7000]})
        true_num_snps_part = [1250, 1251, 499]
        true_ld_bim = pd.DataFrame({'CHR': [1] * 3000,
                                    'POS': list(range(0, 6000, 2)),
...
class Test_LDmatrixBED(unittest.TestCase):
    def test_one_snp(self):
        ...
        ldinfo = pd.DataFrame({'CHR': [21],
                               ...
                               'block_idx2': [0]})
        prop = 0.8
        true_ldinfo = pd.DataFrame({'CHR': [21],
                                    'SNP': ['rs1148733'],
                                    'CM': [0],
                                    'POS': [14677301],
                                    'A1': ['C'],
